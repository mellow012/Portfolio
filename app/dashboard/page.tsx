...
function AddProjectTab({ onSuccess }: { onSuccess: () => void }) {
  ...
  const handleScreenshotsChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const oversized = toProcess.filter((f) => f.size > 3 * 1024 * 1024)
    if (oversized.length) {
      toast(`${oversized.length} file(s) over 3 MB skipped`, 'error')
    }

    const valid = toProcess.filter((f) => f.size <= 3 * 1024 * 1024)
    let loaded = 0
  ...
  const resetForm = () => {
    ...
    setTechInput(''); setScreenshots([]); setScreenshotFiles([]); removeImage()
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (imageFile) {
        const coverPath = `projects/${Date.now()}-${imageFile.name}`
        coverImageUrl = await uploadImageToStorage(imageFile, coverPath)
      }
      ...
      if (screenshotFiles.length > 0) {
        screenshotUrls = await Promise.all(
          screenshotFiles.map((file, idx) => {
            const screenshotPath = `projects/${Date.now()}-screenshot-${idx}-${file.name}`
            return uploadImageToStorage(file, screenshotPath)
          })
        )
      }
...
function EditProjectTab({ projectId, onSuccess }: { projectId: string; onSuccess: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (imageFile) {
        const coverPath = `projects/${Date.now()}-${imageFile.name}`
        finalImageUrl = await uploadImageToStorage(imageFile, coverPath)
      }
      ...
      if (screenshotFiles.length > 0) {
        ...
        // Add new urls to existing screenshots (old base64 ones are filtered out)
        screenshotUrls = screenshots
          .filter((s) => s.startsWith('http')) // Keep only URL screenshots
          .concat(newScreenshotUrls)
      }
      ...
      <form onSubmit={handleSubmit} className="space-y-8 max-w-3xl">
        ...
        <section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            <input placeholder="GitHub URL" value={githubUrl} onChange={(e) => setGithubUrl(e.target.value)} className={inputCls} />
            <input placeholder="Live URL" value={liveUrl} onChange={(e) => setLiveUrl(e.target.value)} className={inputCls} />
            <textarea placeholder="Summary (short description)" value={summary} onChange={(e) => setSummary(e.target.value)} rows={3} className={`${inputCls} sm:col-span-2 resize-none`} />
            <textarea placeholder="Description (detailed)" value={description} onChange={(e) => setDescription(e.target.value)} rows={4} className={`${inputCls} sm:col-span-2 resize-none`} />
            <input placeholder="Technologies (comma-separated)" value={techInput} onChange={(e) => setTechInput(e.target.value)} className={`${inputCls} sm:col-span-2`} />
          </div>
        </section>

        {/* ── Cover Image ── */}
        <section className="glass rounded-2xl p-6">
          <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-widest mb-5">Cover Image</h3>
          {imageUrl && !imageFile && (
            <div className="mb-4 relative">
              <img src={imageUrl} alt="Cover" className="w-full h-48 object-cover rounded-xl" />
              <button type="button" onClick={removeImage} className="absolute top-2 right-2 p-1.5 bg-black/50 hover:bg-black rounded-lg">
          ...
          {imageBase64 && (
            <div className="mb-4 relative">
              <img src={imageBase64} alt="Preview" className="w-full h-48 object-cover rounded-xl" />
              <button type="button" onClick={removeImage} className="absolute top-2 right-2 p-1.5 bg-black/50 hover:bg-black rounded-lg">
                <X className="h-4 w-4 text-white" />
              </button>
            </div>
          )}
          <label className="group block p-6 border-2 border-dashed border-border rounded-2xl text-center cursor-pointer hover:border-primary hover:bg-accent transition">
            <Upload className="h-5 w-5 text-muted-foreground group-hover:text-primary mx-auto mb-2" />
            <p className="text-sm"><span className="text-foreground font-medium">Click to upload</span> or drag and drop</p>
            <p className="text-xs text-muted-foreground">PNG, JPG up to 3 MB</p>
        ...
        <section className="glass rounded-2xl p-6">
          <div className="flex items-center justify-between mb-5">
            <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-widest">Screenshots ({screenshots.length}/{MAX_SCREENSHOTS})</h3>
          </div>
          {screenshots.length > 0 && (
            <div className="grid grid-cols-2 gap-3 mb-4">
              {screenshots.map((shot, i) => (
                <div key={i} className="relative group">
                  <img src={shot} alt={`Screenshot ${i + 1}`} className="w-full h-32 object-cover rounded-xl" />
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition flex items-center justify-center gap-1 rounded-xl">
                    {i > 0 && <button type="button" onClick={() => moveScreenshot(i, i - 1)} className="p-1 bg-white/20 hover:bg-white/30 rounded"><ArrowUpRight className="h-3 w-3 text-white rotate-90" /></button>}
                    {i < screenshots.length - 1 && <button type="button" onClick={() => moveScreenshot(i, i + 1)} className="p-1 bg-white/20 hover:bg-white/30 rounded"><ArrowUpRight className="h-3 w-3 text-white -rotate-90" /></button>}
                    <button type="button" onClick={() => removeScreenshot(i)} className="p-1 bg-rose-500/80 hover:bg-rose-600 rounded"><Trash2 className="h-3 w-3 text-white" /></button>
                  </div>
                </div>
              ))}
            </div>
          ...
          {screenshots.length < MAX_SCREENSHOTS && (
            <label className="group block p-6 border-2 border-dashed border-border rounded-2xl text-center cursor-pointer hover:border-primary hover:bg-accent transition">
              <Upload className="h-5 w-5 text-muted-foreground group-hover:text-primary mx-auto mb-2" />
              <p className="text-sm"><span className="text-foreground font-medium">Add screenshots</span></p>
              <p className="text-xs text-muted-foreground">Up to {MAX_SCREENSHOTS - screenshots.length} more, 3 MB each</p>
              <input type="file" ref={screenshotInputRef} accept="image/*" multiple onChange={handleScreenshotsChange} className="hidden" />
            </label>
          )}
        </section>
        ...
        <div className="pt-2">
          ...
          >
            {isSubmitting ? (
              ...
              <><Edit className="h-4 w-4" /> Update project</>
            )}
          </button>
        </div>
      </form>
